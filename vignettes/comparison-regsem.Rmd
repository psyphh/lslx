...
* **lslx** and **regsem** yield different results;
* **lslx** is about 25 times faster than **regsem**.
...
By the fact that the objective function values made by **lslx** and **lsl** are smaller then that made by **regsem**, we suspect that the current version of **regsem** (0.9.2) may not find an optimal solution.
...
```
By the fact that the objective value made by **regsem** is larger than that made by **lslx** It seems that the **regsem** doesn't find an optimal solution.
We can also justify the optimality of fitting result of **lslx** by the sub-gradient of objective function
```{r comment = "", message = FALSE}
...
```
We can observe that the estimates are quite different.
...
```
The objective function value and estimate made by **lsl** is consistent with **lslx**, which further supports that **regsem** doesn't find an optimal solution.
...
microbenchmark::microbenchmark(
  ...
  times = 10)
```
We can see that **lslx** is about 25 times faster than **regsem**.
