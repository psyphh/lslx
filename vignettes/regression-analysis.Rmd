...
The data is stored in a `data.frame` named `data`.
...
The following code summarize the fitting result under the penalty level selected by Akaike information criterion.
```{r comment = "", message = FALSE, fig.width = 24, fig.height = 14}
r6_lslx$summarize(selector = "aic")
```
In this example, we can observed that all of the penalized coefficients are identified as zero, which is consistent with their population values. 
...
The plot shows that an optimal value of lambda is `0.15`.
```{r comment = "", message = FALSE, fig.width = 8, fig.height = 4, dpi=300, out.width=600, out.height=300}
...
We can see that all the coefficients become zero when the value of lambda is larger than `0.15`.
```{r comment = "", message = FALSE, fig.width = 8, fig.height = 4, dpi=300, out.width=600, out.height=300}
...
For example, the coefficient can be obtained by
```{r comment = "", message = FALSE, fig.width = 8, fig.height = 4, dpi=300, out.width=600, out.height=300}
r6_lslx$extract_coefficient(selector = "aic")
```

We may also check the quality of optimization by viewing the subgradient of objective function
```{r comment = "", message = FALSE, fig.width = 8, fig.height = 4, dpi=300, out.width=600, out.height=300}
r6_lslx$extract_objective_gradient(selector = "aic")
```
The subgradient shows that the obtained solution is optimal since all the elements are very small.
